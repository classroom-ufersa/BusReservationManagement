...
def excluir_reserva(nome):
    ...
    if deleted:
        print(f"A reserva para {nome} foi excluída com sucesso.")
    else:
        print(f"Não foi encontrada uma reserva para {nome}.")
def listar_reservas():
    with open('tickets.txt', 'r') as file:
        lines = file.readlines()
    ...
    if len(lines) == 0:
        ...
        for i in range(0, len(lines), 4):
            ...
            print(f"Número do ônibus selecionado: {num_bus}")
            print(f"Número do ticket: {num_ticket}")
            print()
